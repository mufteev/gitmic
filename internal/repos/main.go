...
func RunConcurrency(isPrintData bool) error {
	...
	//   Из-за чего обновлённые HTTP-запросы `http.NewRequestWithContext` будут завершаться до заавершения HTTP-запроса
	...
	for i := range *repos {
		...
		// По этой же причине мы не будем использовать второй параметр цикла содержащего в себе
		ii := i
